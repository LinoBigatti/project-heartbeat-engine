...
for file in sys.argv[1:]:
    ...
    prefix = ""
    if "platform" in split:
        print(split)
    if len(split) >= 3 and split[-3] == "modules" and split[-1] == "register_types.h":
        prefix = f"{split[-2]}_"  # Name of module.
    elif split[1] == "platform" and (file.endswith("api/api.h") or "/export/" in file):
        prefix = f"{split[2]}_"  # Name of platform.
    elif file.startswith("engine/modules/mono/utils") and "mono" not in split[-1]:
        prefix = "MONO_"
    elif file == "engine/servers/rendering/storage/utilities.h":
        prefix = "RENDERER_"
